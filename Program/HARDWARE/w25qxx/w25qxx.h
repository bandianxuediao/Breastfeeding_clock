...
typedef struct
{
	...
	u8	CARD_NUM[8];
	u8	BOX_KEY[3];
	u8	BOX_STATE;//0x01卡存，0x02密存，0x11空
}BOX_INFO;
...
extern u8	 W25QXX_ReadSR(void);        		//读取状态寄存器 
extern void W25QXX_Write_SR(u8 sr);  			//写状态寄存器
...
extern void Write_ID_W25Q(u8 addr , u8*ID_BUFF ); //ADDR占用1个字节最大255
