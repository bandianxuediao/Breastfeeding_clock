...
//要写入到W25Q64的字符串数组
 u8 W25Q_ADD_ID[];
u32	FLASH_SIZE=128*1024*1024;	//FLASH 大小为128M位 16M字节
u16 W25QXX_TYPE=W25Q128;	//默认是W25Q128

//			W25QXX_Write((u8*)TEXT_Buffer,FLASH_SIZE-400,SIZE);			//从倒数第100个地址处开始,写入SIZE长度的数据

//			W25QXX_Read(datatemp,j*100+(FLASH_SIZE-4000),SIZE);					//从倒数第100个地址处开始,读出SIZE个字节

/*将卡号存入spi_flash
		卡号存储从FLASH_SIZE-1000开始，每个卡占用50bit，最大200张卡
		卡索引存储从100开始，每个索引占3字节，存储snr的第一第四位
*/
void Write_ID_W25Q(u8 addr , u8*ID_BUFF ) //ADDR占用1个字节最大255
{
	W25Q_ADD_ID[0]=addr;
	...
	W25QXX_Write((u8*)W25Q_ADD_ID,100+addr*10,3);			//从倒数第100个地址处开始,写入id索引
...
u8 Find_Card_Box(void)
{
	...
	{
		W25QXX_Read(&datatemp[12],100+13+i*20,1);					//从第100个地址处开始,读出SIZE个字节
		if(datatemp[12] == 0x01)
		{
			W25QXX_Read(&datatemp[0],100+13+i*20,9);					//从第100个地址处开始,读出SIZE个字节
...
u8 Find_ID_W25Q(u8 *ID_BUFF)
{
	...
	{
		W25QXX_Read(datatemp,100+BOX_SEAT*10,3);					//从第100个地址处开始,读出SIZE个字节
		if(datatemp[1]==ID_BUFF[2]|datatemp[2]==ID_BUFF[5])	
		{
			W25QXX_Read(datatemp,FLASH_SIZE-10000+BOX_SEAT*50,W25Q_SIZE);					//从倒数第100个地址处开始,读出SIZE个字节
