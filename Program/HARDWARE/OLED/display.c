...
//	u8 temp[50];
	switch(state)
	{
		case 1:
			clr_disp_mem();         //清除显存数据
			oled_print(3, LINE0, "存包") ;  //字符输出函数
			oled_print(0, LINE1, "输入箱号") ;  //字符输出函数
			oled_print(0, LINE2, "箱号:") ;  //字符输出函数		
			break;
		case  2:
			
			switch(GetCurrentState())
		{
			case INPUT_BOXNUM:   //判定现在属于什么状态			
				if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}		//如果是输入的第一个数，按照数字和西文输入
				oled_print(INPUT_DATA_AMOUNT/2+3, LINE2, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数
		    break;
			case INPUT_BOXKEY :			
				if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}
				oled_print(INPUT_DATA_AMOUNT/2+3, LINE4, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
				break;
			case SET_KEY_BOXNUM:
					if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}
					oled_print(INPUT_DATA_AMOUNT/2+3, LINE0, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数
				break;
			case SET_KEY_OLD_BOXKEY :
						if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}
						oled_print(INPUT_DATA_AMOUNT/2+3, LINE1, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
				break;
			case SET_KEY_NEW_BOXKEY:
						if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}
						oled_print(INPUT_DATA_AMOUNT/2+3, LINE2, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
				break;
		 	case SET_KEY_SURE_BOXKEY :
						if(INPUT_DATA_AMOUNT==1){new_front_state = 0;}
						oled_print(INPUT_DATA_AMOUNT/2+3, LINE3, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
					break;
			case INPUT_CARD_KEY://刷卡存包输入密码
						INPUT_PASS_STATE = 1;
						if(INPUT_DATA_AMOUNT==1){new_front_state = 1;}
						if(INPUT_DATA_AMOUNT==2){new_front_state = 0;}						
						oled_print(INPUT_DATA_AMOUNT/3+4, LINE2, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数	
//						oled_print((INPUT_DATA_AMOUNT+1)/3+4, LINE2, "|") ;  //字符输出函数
					break;
			case INPUT_CARD_KEY_AGAIN://刷卡存包再次输入密码
						INPUT_PASS_STATE = 2;
						if(INPUT_DATA_AMOUNT==1){new_front_state = 1;}
						if(INPUT_DATA_AMOUNT==2){new_front_state = 0;}						
						oled_print(INPUT_DATA_AMOUNT/3+5, LINE3, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
					break;
			case OUTPUT_CARD_KEY://刷卡取包输入密码
						INPUT_PASS_STATE = 3;
						if(INPUT_DATA_AMOUNT==1){new_front_state = 1;}
						if(INPUT_DATA_AMOUNT==2){new_front_state = 0;}						
						oled_print(INPUT_DATA_AMOUNT/3+4, LINE2, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
					break;
			case PASSW_INPUT_KEY://密码存包输入密码
						INPUT_PASS_STATE = 4;
						if(INPUT_DATA_AMOUNT==1){new_front_state = 1;}
						if(INPUT_DATA_AMOUNT==2){new_front_state = 0;}						
						oled_print(INPUT_DATA_AMOUNT/3+4, LINE2, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数	
//						oled_print((INPUT_DATA_AMOUNT+1)/3+4, LINE2, "|") ;  //字符输出函数
					break;
			case PASSW_INPUT_KEY_AGAIN://密码存包再次输入密码
						INPUT_PASS_STATE = 5;
						if(INPUT_DATA_AMOUNT==1){new_front_state = 1;}
						if(INPUT_DATA_AMOUNT==2){new_front_state = 0;}						
						oled_print(INPUT_DATA_AMOUNT/3+5, LINE3, &INPUT_DATA_NUM[INPUT_DATA_AMOUNT-1]) ;  //字符输出函数			
			...
			break;
		case	3:
			oled_print(0, LINE3, "设置密码") ;  //字符输出函数
			oled_print(0, LINE4, "密码:") ;  //字符输出函数
			break;
		case 4:
			clr_disp_mem();         //清除显存数据
			oled_print(2, LINE0, "修改密码") ;  //字符输出函数
			oled_print(0, LINE1, "箱号:") ;  //字符输出函数
			oled_print(0, LINE2, "旧密码:") ;  //字符输出函数
			oled_print(0, LINE3, "新密码:") ;  //字符输出函数
			oled_print(0, LINE4, "确认密码:") ;  //字符输出函数				
			break;
...
//		case 2:
//			sprintf((char*)temp,"位置在:%d",BOX_SEAT);
//			oled_print(0, LINE3, &temp[0]);//字符输出
//			break;
		default:
			break;
...
void ShowSys_Message(u8 state)
{
	...
	{
		case 0:
			oled_print(3, LINE2, "请稍等...") ;  //字符输出函数
			break;
		
		case 1:
			oled_print(0, LINE1, "设备初始化失败！") ;  //字符输出函数
			break;
		case 2:
			oled_print(0, LINE0, "直接确认则无密码") ;  //字符输出函数
			sprintf((char*)temp,"箱 号:%d",USER_BOX_NUM);
			oled_print(0, LINE1, &temp[0]);//字符输出
			oled_print(0, LINE2, "密 码:") ;  //字符输出函数
			oled_print(0, LINE3, "密码确认:") ;  //字符输出函数		
			oled_print(0, LINE4, "      确认") ;  //字符输出函数		
			break;
		case 3:
			sprintf((char*)temp," %d号箱子已打开",USER_BOX_NUM);
			oled_print(0, LINE2, &temp[0]);//字符输出
			break;
		case 4:
			oled_print(0, LINE1, " 密码不一致");//字符输出
			oled_print(0, LINE2, "  重新输入");//字符输出
			break;
		case 5:
			oled_print(0, LINE1, "请输入密码");//字符输出
			oled_print(0, LINE2, "密 码:");//字符输出
			oled_print(0, LINE4, "      确认") ;  //字符输出函数		
			break;
		case 6:
			oled_print(0, LINE1, "刷卡异常");//字符输出
			oled_print(0, LINE2, "请联系管理员");//字符输出
			break;
		case 7:
			oled_print(0, LINE0, "   存包") ;  //字符输出函数
			sprintf((char*)temp,"箱 号:%d",USER_BOX_NUM);
			oled_print(0, LINE1, &temp[0]);//字符输出
			oled_print(0, LINE2, "密 码:") ;  //字符输出函数
			oled_print(0, LINE3, "密码确认:") ;  //字符输出函数		
			...
			break;
		case 8:
			oled_print(0, LINE0, "  请输入箱号") ;  //字符输出函数
			oled_print(0, LINE2, " 箱号:") ;  //字符输出函数
			oled_print(0, LINE4, "      确认") ;  //字符输出函数		
			break;
		case 9:
			oled_print(0, LINE1, "箱号异常");//字符输出
			oled_print(0, LINE2, "请联系管理员");//字符输出
			break;
		case 10:
			oled_print(1, LINE1, "此箱需刷卡");//字符输出
			oled_print(1, LINE2, "请刷卡开箱");//字符输出
			break;
		case 11:
			oled_print(0, LINE0, "   取包") ;  //字符输出函数
			sprintf((char*)temp,"箱 号:%d",USER_BOX_NUM);
			oled_print(0, LINE1, &temp[0]);//字符输出
			oled_print(0, LINE2, "密 码:") ;  //字符输出函数
			oled_print(0, LINE4, "      确认") ;  //字符输出函数		
			break;	
		case 12:
			oled_print(0, LINE1, "此箱为空");//字符输出
			break;
		case 13:
			oled_print(0, LINE1, "箱号不存在");//字符输出
			break;
...
void ShowPad(u8 state)
{
	...
	{
		case 0:
			oled_print(0, LINE1, "01234") ;  //字符输出函数
			break;
		case 1:
			oled_print(0, LINE1, temp) ;  //字符输出函数
			break;
		case 2:
			oled_print(0, LINE1, "2键被按下") ;  //字符输出函数
			break;
		case 3:
			oled_print(0, LINE1, "3键被按下") ;  //字符输出函数
			break;
		case 4:
			oled_print(0, LINE1, "4键被按下") ;  //字符输出函数
			break;
		case 5:
			oled_print(0, LINE1, "5键被按下") ;  //字符输出函数
			break;
		case 6:
			oled_print(0, LINE1, "6键被按下") ;  //字符输出函数
			break;
		case 7:
			oled_print(0, LINE1, "7键被按下") ;  //字符输出函数
			break;
		case 8:
			oled_print(0, LINE1, "8键被按下") ;  //字符输出函数
			break;
		case 9:
			oled_print(0, LINE1, "9键被按下") ;  //字符输出函数
			break;
		case 10:
			oled_print(0, LINE1, "IN键被按下") ;  //字符输出函数
			break;
		case 11:
			oled_print(0, LINE1, "OUT键被按下") ;  //字符输出函数
			break;
		case 12:
			oled_print(0, LINE1, "SYS键被按下") ;  //字符输出函数
			break;
		case 13:
			oled_print(0, LINE1, "KEY键被按下") ;  //字符输出函数
			break;
		case 14:
			oled_print(0, LINE1, "BACK键被按下") ;  //字符输出函数
			break;
		case 15:
			oled_print(0, LINE1, "SURE键被按下") ;  //字符输出函数
			break;
	...
	switch(state)
	{
		case 0:
			...
			InputDataInit(3, 3, 3, 0);
				...
				show_status_bar("按↙确认",ALIGN_CENTER,"",ALIGN_LEFT);//在显示屏底部显示状态信息
